...
function _errorCheck(value) {
    if (_isError(value)) {
        if (typeof Error.captureStackTrace === "function") {
            Error.captureStackTrace(assert, value);
        }
        throw value;
    }
}
...
async function rejects(block, Error_opt, message) {
    ...
    if (Error_opt) {
        const errorMatches = (typeof Error_opt === "function" && rejectedError instanceof Error_opt)
            || (typeof Error_opt === "string" && typeof rejectedError?.message === "string")
...
function match(string, regexp, message) {
    if (typeof string !== "string") {
        _errorCheck(message);
        throw new TypeError(`[match] TypeError: " ${string} is not a string${message ? " - " + _toSafeString(message) : ""}`);
    }
    if (!(regexp instanceof RegExp)) {
        _errorCheck(message);
        throw new TypeError(`[match] TypeError: ${_toSafeString(regexp)} is not a RegExp${message ? " - " + _toSafeString(message) : ""}`);
    }
...
function doesNotMatch(string, regexp, message) {
    if (typeof string !== "string") {
        _errorCheck(message);
        throw new TypeError(`[doesNotMatch] TypeError: " ${string} is not a string${message ? " - " + _toSafeString(message) : ""}`);
    }
    if (!(regexp instanceof RegExp)) {
        _errorCheck(message);
        throw new TypeError(`[doesNotMatch] TypeError: ${_toSafeString(regexp)} is not a RegExp${message ? " - " + _toSafeString(message) : ""}`);
    }
    if (regexp.test(String(string))) {
        _errorCheck(message);
...
function inRange(value, min, max, message) {
    if (!_inRange(value, min, max)) {
        _errorCheck(message);
        let errorMessage = `[inRange] Assertion failed: ${_toSafeString(value)} should be in range ${_toSafeString(min)} and ${_toSafeString(max)} or the type of the values are not the same${message ? " - " + _toSafeString(message) : ""}`;
        throw new assert.AssertionError(errorMessage, {
            message: errorMessage,
            cause: errorMessage,
            actual: value,
...
function notInRange(value, min, max, message) {
    if (_inRange(value, min, max)) {
        _errorCheck(message);
        let errorMessage = `[notInRange] Assertion failed: ${_toSafeString(value)} should be not in range ${_toSafeString(min)} and ${_toSafeString(max)}${message ? " - " + _toSafeString(message) : ""}`;
        throw new assert.AssertionError(errorMessage, {
            message: errorMessage,
            cause: errorMessage,
            actual: value,
...
function includes(container, options, message) {
    if (typeof options !== "object") {
        _errorCheck(message);
        throw new TypeError(`[includes] TypeError: ${_toSafeString(options)} is not an object${message ? " - " + _toSafeString(message) : ""}`);
    }
...
function doesNotInclude(container, options, message) {
    if (typeof options !== "object") {
        _errorCheck(message);
        throw new TypeError(`[doesNotInclude] TypeError: ${_toSafeString(options)} is not an object${message ? " - " + _toSafeString(message) : ""}`);
    }
    if (_includes(container, options.keyOrValue, options?.value ?? undefined)) {
        _errorCheck(message);
...
function testSync(block, name = "assert.testSync") {
    try {
...
async function testAsync(block, name = "assert.testAsync") {
    try {
