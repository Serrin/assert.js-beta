...
function _is(value, expectedType, caller = "is") {
    function _matches(value, expected) {
        if (typeof expected === "string") {
            return _typeOf(value) === expected;
        }
        try {
    ...
    if (Array.isArray(expectedType)) {
        ...
        for (const item of expectedType) {
            if (typeof item !== "string" && typeof item !== "function") {
                throw new TypeError(`[${_str(caller)}] TypeError: expectedType array elements must be string or function. Got ${_typeOf(item)}`);
            }
        }
        return expectedType.some((item) => _matches(value, item));
    }
    throw new TypeError(`[${_str(caller)}] expectedType array elements must be strings or constructors. Got ${_typeOf(expectedType)}`);
}
function _str(v) {
...
const equal = (actual, expected, message) => operator(actual, assert.config.alwaysStrict ? "Object.is" : "==", expected, message);
const notEqual = (actual, expected, message) => operator(actual, assert.config.alwaysStrict ? "!Object.is" : "!=", expected, message);
const strictEqual = (actual, expected, message) => operator(actual, "Object.is", expected, message);
const notStrictEqual = (actual, expected, message) => operator(actual, "!Object.is", expected, message);
...
function operator(value1, operatorStr, value2, message) {
    oneOf(operatorStr, ["==", "!=", "===", "!==", "<", "<=", ">", ">=", "Object.is", "!Object.is"], message);
    let result = false;
    switch (operatorStr) {
        case "==":
